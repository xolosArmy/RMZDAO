...
import { getConfidence, getDefaultServers, getUserAgent, } from "./configuration.js";
import { parseElectrumUrl } from "./util.js";
import { Network } from "../interface.js";
import { networkTickerMap, clusterParams, ELECTRUM_CASH_PROTOCOL_VERSION, } from "./constant.js";
...
export function getNetworkProvider(network = Network.MAINNET, servers, manualConnectionManagement, options) {
    ...
    if (servers) {
        ...
        let provider = new ElectrumNetworkProvider(clusterOrClient, network, manualConnectionManagement);
        if (globalContext) {
...
function getElectrumCluster(params, network) {
    return new ElectrumCluster(getUserAgent(), ELECTRUM_CASH_PROTOCOL_VERSION, params.confidence, params.distribution, params.order, params.timeout);
}
function getElectrumClient(params, timeout, network) {
    return new ElectrumClient(getUserAgent(), ELECTRUM_CASH_PROTOCOL_VERSION, params.host, params.port, params.scheme, timeout);
}

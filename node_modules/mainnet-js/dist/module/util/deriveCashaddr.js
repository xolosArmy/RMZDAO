import { encodeCashAddress, CashAddressType, secp256k1, decodeCashAddressFormat, decodeCashAddressFormatWithoutPrefix, cashAddressTypeBitsToType, decodeCashAddressVersionByte, } from "@bitauth/libauth";
...
export function toCashaddr(tokenaddr) {
    let result;
    ...
    return encodeCashAddress(result.prefix, CashAddressType.p2pkh, result.payload);
}
export function toTokenaddr(cashaddr) {
    let result;
    // If the address has a prefix decode it as is
    ...
    if (typeof result === "string")
        throw new Error(result);
    return encodeCashAddress(result.prefix, CashAddressType.p2pkhWithTokens, result.payload);
}

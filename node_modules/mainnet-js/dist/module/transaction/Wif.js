import { authenticationTemplateP2pkhNonHd, authenticationTemplateToCompilerBCH, cashAddressToLockingBytecode, encodeTransaction, generateTransaction, importAuthenticationTemplate, hexToBin, verifyTransactionTokens, decodeTransaction, } from "@bitauth/libauth";
...
export async function buildP2pkhNonHdTransaction({ inputs, outputs, signingKey, sourceAddress, fee = 0, discardChange = false, slpOutputs = [], feePaidBy = FeePaidByEnum.change, changeAddress = "", }) {
    if (!signingKey) {
    ...
    const tokenValidationResult = verifyTransactionTokens(result.transaction, sourceOutputs);
...
export async function getFeeAmountSimple({ utxos, sendRequests, relayFeePerByteInSatoshi, slpOutputs, discardChange, }) {
...
export async function getFeeAmount({ utxos, sendRequests, privateKey, sourceAddress, relayFeePerByteInSatoshi, slpOutputs, feePaidBy, discardChange, }) {
    // build transaction
    if (utxos) {
...
export async function buildEncodedTransaction({ inputs, outputs, signingKey, sourceAddress, fee = 0, discardChange = false, slpOutputs = [], feePaidBy = FeePaidByEnum.change, changeAddress = "", buildUnsigned = false, }) {

 ...
 */
export const generateRandomBytes = (length) => crypto.getRandomValues(new Uint8Array(length));
/**
 ...
 */
export var DeterministicEntropyGenerationError;
(function (DeterministicEntropyGenerationError) {
    DeterministicEntropyGenerationError["insufficientEntropy"] = "Deterministic entropy generation error: the provided list of events contains insufficient entropy.";
})(DeterministicEntropyGenerationError || (DeterministicEntropyGenerationError = {}));
/**
 ...
 * to use
 */
export const generateDeterministicEntropy = (possibleResults, events, requiredEntropyBits = 128 /* KeyUtilConstants.privateKeyRequiredEntropyBits */, crypto = { sha256: internalSha256 }) => {
    if (events.length <
    ...
    const source = Uint8Array.from(events.map((v) => v + 48 /* KeyUtilConstants.utf8NumbersOffset */));
